...
In the discussion below, I derive the female best-response function given some platform state $\Psi$ and male strategy $\mu$. 
Consider now a woman $i$ who is paired with a man $j$ in the platform. 
Given that $j$'s action depends on his own budget, which is unknown to woman $i$, then, under strict rationality, woman $i$ would have to compute her expected payoff conditional on her beliefs for the market history. 
This behaviour is both unreasonable and intractable in an SBDP setting, as explained in \autoref{sec:section1.1}; instead, given a continuum of agents and anonymous interactions, it is reasonable to conjecture that the average swiping rate for men is stationary, and any individual agent's actions has a negligible effect on the platform state dynamics.
...
Here, $\overline\mu$ denotes the average swiping rate for men, thus imposing a mean-field assumption such that woman $i$ conditions her expectation for $j$'s action on the invariant distribution of male budgets in $\Psi$. 
This modelling choice, which has been employed by \cite{immorlica2021designing} and \cite{iyer2014mean} among others, simplifies the full dynamic game by collapsing it onto a pair of Markov Decision Processes (MDPs), where strategy $\omega$ is a best response for female agents if, and only if, it is an optimal policy for the corresponding MDP. 
Furthermore, this simplification also reduces the state of payoff-dependent variables for $i$ to just the value $\theta$ that she observes for $j$ and her own budget $b$. 
Therefore, since all agents in the same side solve the same MDP, which encompasses only the above state variables, this restricts the paper's focus onto the set of (pure) symmetric stationary strategies, denoted by $\mu: \Theta \times\mathcal{B}_m\rightarrow \mathcal{A}_m$ for men and $\omega:\Theta \times\mathcal{B}_w\rightarrow \mathcal{A}_w$ for women.
