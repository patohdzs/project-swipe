...
This last property stems from the fact that, in most SBDPs, users are allowed to unmatch with each other, therefore matching with the least attractive individual on the other side of the market is weakly preferred to not matching. 
...
With the model framework and market dynamics outlined above, I now present the decision problem faced by female agents in the market, with analogous results and implications for the male side. In the discussion below, I derive the female best-response function given a fixed, stationary market state $\Psi$ and male strategy $\mu$. To begin this analysis, consider a woman $i$ who is paired with a man $j$ in the platform. The expected ex-interim payoff for this women, given that she observes attractiveness $\theta$ for candidate $j$ and chooses action $a$, is the following: 
\begin{equation*}
    \begin{aligned}
...
Even though the reservation attractiveness levels can be computed using numerical methods, such as value or policy iteration, these are more explicitly characterised by the result below (with a corresponding proof included in \autoref{appx: b}): 
...
The result in \autoref{prop:recurrence relation} allows for an improved computation of agent policies relative to the aforementioned algorithms, which have a runtime complexity of $\mathcal{O}(|\mathcal{A}||\Theta \times \mathcal{B}_w|^2)$ and can become expensive for both large budget sets and fine discretisations of $\Theta$. Additionally, by inspecting this result, it is evident that the averaged behaviour of the opposite side of the market ($\overline\mu$) has no direct influence over female best responses. Instead, this influence happens indirectly through the steady state masses and their effect on how agents discount inter-temporal payoffs. Using the recurrence relation in \autoref{prop:recurrence relation}, the agent-best response was computed for an arbitrary set of exogenous parameters, with results shown in \autoref{fig:swiping-rule}. Here, the optimal policy for agents is marked by a clear cut-off rule for swiping right. These cut-off values are decreasing in the agent's budget, which captures the notion that an agent's current swipe is more valuable than all preceding ones given the increasing opportunity cost.  
