...
Since gender imbalances can leave some agents in the long side of the market unpaired, a pairings process must also be determined. Given the automated nature of SBDPs, I assume an efficient matching technology and model pairings as a Bernoulli process parametrised by market tightness; thus, the probability of being paired with a candidate is defined for both sides as:  
\begin{equation*}
...
The above imposes a mean-field assumption, such that woman $i$ accounts for $j$'s behaviour only through $\overline\mu$ conditioned on the steady-state $\Psi$.
...
Let the jump times of the realised pairing process for woman $i$ be index by $k$. 
...
Here, the reservation values $\{\widetilde\omega\}_{b\in \mathcal{B}_w}$ are such that woman $i$ is indifferent between swiping left or right. Therefore, an optimal policy for woman $i$'s MDP could involve swiping right for partners who exceed the reservation value for her current budget:  
...
Strictly speaking, there is an infinite number of optimal policies for this MDP that involve any action mix at the indifference point $\widetilde\omega_b$, but I refrain from further discussion of this technicality given that there is a measure zero of agents for which $i$ observes such reservation value. Using both of these results, I derive the following explicit characterisation for $\{\widetilde\omega_b\}_{b\in \mathcal{B}_w}$ (with a corresponding proof included in \autoref{appx: b}):  
