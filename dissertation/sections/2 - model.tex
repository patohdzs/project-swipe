...
With the model framework outlined above, I now present the decision problem faced by female agents in the market, with analogous results and implications for the male side. In the discussion below, I derive the female best-response function given some platform steady-state $\Psi$ and male strategy $\mu$. Consider now a woman $i$ who is paired with a man $j$ in the platform. The expected ex-interim payoff for this women, given that she observes attractiveness $\theta$ for candidate $j$ and chooses action $a$, is the following: 
\begin{equation*}
    \begin{aligned}
...
\begin{comment}
    % What would traditional GT do?
    Given that $j$'s action depends on his own budget, which is unknown to woman $i$, rationality in the strictest sense would have woman $i$ compute her expected payoff conditional on her beliefs for the market history. This behaviour is both unreasonable and intractable in markets with a large number of agents, as explained in \autoref{sec:section1.1}. Instead, large markets 
    
    
    
    As such, the expected ex-interim payoff woman $i$, given that she observes attractiveness $\theta$ for candidate $j$ and chooses action $a$, is the following: 
    
    % Here, $\overline\mu$ imposes a mean-field assumption that allows woman $i$ to condition her expectation for $j$'s action on the invariant distribution of male budgets in $\Psi$. This modelling choice, which has been employed by \cite{immorlica2021designing} and \cite{iyer2014mean} among others, simplifies the full dynamic game by collapsing it onto a pair of Markov Decision Processes (MDPs), where strategy $\omega$ is a best response for female agents if and only if it is an optimal policy for the corresponding MDP.

    Due to the anonymity of interactions in the platform
\end{comment}

Here, $\overline\mu$ denotes candidate $j$'s strategy averaged over the possible attractiveness that he may observe for $i$ and his possible budget level, both of which are unknown to woman $i$. The payoff function above imposes a mean-field assumption such that, conditional on the platform state $\Psi$, woman $i$ considers only the empirical frequency with which she receives a right-swipe. This modelling choice, which has been employed by \cite{immorlica2021designing} and \cite{iyer2014mean} among others, simplifies the full dynamic game by collapsing it onto a pair of Markov Decision Processes (MDP's), where strategy $\omega$ is a best response for female agents if and only if it is an optimal policy for the corresponding MDP. Let the jump times of the realised pairing process for woman $i$ be index by $k$. Given that, at the time of pairing, this women has a budget of $b$ right swipes left, she then solves the constrained MDP presented below, captured by the value function $V_w(\theta,b)$: 
\begin{equation*}
    \begin{aligned} 
